...
class PlotTests(unittest.TestCase):
    ...
    def test_scatter(self):
        """Create scatter plot"""
        fig_df = Plot(
            outpath=self.outpath, save_image=self.save_image
        ).scatter(self.df, outfile="scatter_df.png")
        fig_list = Plot(
            outpath=self.outpath, save_image=self.save_image
        ).scatter(self.list, outfile="scatter_list.png")
    ...
    def test_series(self):
        ...
        fig_list = Plot(
            outpath=self.outpath, save_image=self.save_image
        ).series(self.list, outfile="series_list.png")

    def test_box(self):
        """Create box plot"""

        df1 = pd.DataFrame(
            data=np.random.rand(120, 2), columns=["df1_1", "df1_2"]
        )
        df2 = pd.DataFrame(
            data=np.random.rand(100, 2) + 0.2, columns=["df2_1", "df2_2"]
        )
        df3 = pd.DataFrame(
            data=np.random.rand(80, 2) + 0.4, columns=["df3_1", "df3_2"]
        )
