...
class ConverterTests(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.t_col_in = UnitConv(35.).degC_K(unit_in='degC')  # [K]

        # for single time step
        self.inc_rad = 1000.  # [W/m2]
        self.t_amb = UnitConv(25.).degC_K(unit_in='degC')  # [K]

        # PV size (the parameters will follow below)
        pv_size = 1000.

        sizes = pd.DataFrame(
            data=[[self.s['sol_col'], self.gross_area],
                  [self.s['pv'], pv_size]],
            columns=[self.s['comp'], self.s['cap']])

        sol_cd_params = pd.DataFrame(
            data=[[self.s['sol_col'], self.s['interc_cd'], .75],
                  [self.s['sol_col'], self.s['a1_cd'], -3.688],
                  [self.s['sol_col'], self.s['a2_cd'], -.0055]],
            columns=[self.s['comp'], self.s['param'],
                     self.s['param_value']])

        sol_hwb_params = pd.DataFrame(
            data=[[self.s['sol_col'], self.s['interc_hwb'], .753],
                  [self.s['sol_col'], self.s['slope_hwb'], -4.025]],
            columns=[self.s['comp'], self.s['param'],
                     self.s['param_value']])

        pv_simple_params = pd.DataFrame(
            data=[[self.s['pv'], self.s['eta_pv'], .16],
        ...
        self.hwb_col = Converter(params=sol_hwb_params,
                                 sizes=sizes)
        self.cd_col = Converter(params=sol_cd_params,
                                sizes=sizes)
        self.simple_pv_kWpeak = Converter(params=pv_simple_params,
                                          sizes=sizes)

        self.simple_pv_area = Converter(params=pv_simple_params,
                                        sizes=sm_pv_area)

        self.hp = Converter(params=self.hp_params,
                            weather=mild,
                            sizes=sizes)

        self.inst_gas_wh = Converter(params=gas_burn_params,
                                     sizes=gas_burn_size)

        self.el_res = Converter(params=el_res_params,
                                sizes=el_res_size)

        # functional test for initiation with default inputs
        self.params_none = Converter(params=None,
                                     sizes=1.)
    ...
    def test_electric_resistance(self):
        ...
        """
        demand = np.array([8000., 6500., 3000.])
        delivery = np.array([6500., 6500., 3000.])
    ...
    def test__heat_pump(self):
        ...
        performance = (C1 + C2 * T_wet_bulb_C + C3 * T_wet_bulb_C *
                       T_wet_bulb_C + C4 * T_tank_C + C5 * T_tank_C *
                       T_tank_C + C6 * T_wet_bulb_C * T_tank_C)
    ...
    def test_heat_pump(self):
        ...
        # Calculate performance factor
        performance_factor = (
            C1 + C2 * T_wet_bulb_C + C3 * T_wet_bulb_C *
            T_wet_bulb_C + C4 * T_tank_C + C5 * T_tank_C *
            T_tank_C + C6 * T_wet_bulb_C * T_tank_C)
    ...
    def test_heat_pump_validate(self):
        ...
        for T_wb_static in T_wb_const_dict.values():
            ...
            cop_res.append(self.hp.heat_pump(
                T_wet_bulb, T_tank)['cop'])
    ...
    def test__heater(self):
        ...
        # array
        demand = np.array([26000., 24000., 20000.])
        delivery = np.array([24000., 24000., 20000.])
        unmet = np.array([2000., 0., 0.])
        consumed = delivery / eta
    ...
    def test_gas_burner(self):
        ...
        self.assertTrue(
            (array_burner[self.r['q_del_bckp']] == delivery).all())
    ...
    def test__hwb_solar_collector(self):
        ...
        gain = (self.gross_area * self.inc_rad * (
                intercept + slope * ((self.t_col_in - self.t_amb) /
                self.inc_rad)))
    ...
    def test__cd_solar_collector(self):
        ...
        gain = (self.gross_area * self.inc_rad * (
                intercept + a_1 * (
                    (self.t_col_in - self.t_amb) / self.inc_rad) +
                a_2 * ((self.t_col_in - self.t_amb) / self.inc_rad**2)))

        self.assertAlmostEqual(
            self.comp._cd_solar_collector(self.gross_area, self.inc_rad,
    ...
    def test_compare_annual_sol_col_perf(self):
        ...
        # under 10%
        both_gain = np.logical_and(self.hwb_col.sol_col_gain,
            self.cd_col.sol_col_gain)
        ...
        err = cd - hwb
        rel_err = abs(err / cd)
        self.assertTrue(np.mean(rel_err) < .1)
    ...
    def test__simple_photovoltaic(self):
        ...
        self.assertEqual(self.comp._simple_photovoltaic(
            irrad,
            panel_area=panel_area,
            f_act=f_act,
            eta_pv=eta_pv,
            eta_dc_ac=eta_dc_ac)['ac'], power)
    ...
    def test_sol_col_validate_with_SAM(self):
        ...
        self.sf = sf.loc[:, [self.c['irrad_on_tilt'],
                             self.c['t_amb_C'],
                             self.c['month']]]
        ...
        err = abs((self.sf.sum()['sol_gain'] / 1000. -
                  ...
                  (self.sf.sum()['sol_gain'] / 1000.))

        # check if the error is less than
        perc = 10.

        self.assertTrue(err < perc / 100., 'Error is {}!'.format(err))

        if self.plot_results:
            # plot duration curves
            Plot(data_headers=['HWB', 'SAM'],
                 outpath=self.outpath,
                 save_image=self.plot_results,
                 title=\
                'HWB and SAM model duration curves',
                 label_v='Solar gain [W/m^2]',
                 duration_curve=True).series(
                     [self.hwb_col.sol_col_gain,
                      1000. * sam_data['solar net gain SAM'].values],
    ...
    def test_pv_validate_with_SAM(self):
        ...
        for case in cases:
            ...
            sam_data = pd.read_csv(os.path.join(
                self.outpath, 'data/sam_pv_' + case + '.csv'))
            ...
            # write to csv file
            pv_yields.to_csv(os.path.join(self.outpath,
                'data/' + csv_file + case + '.csv'))
...
class StorageTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tank = Storage(
            size=UnitConv(30).m3_gal(),
            type='sol_tank')  # .11356236 m3
    ...
    def test__tank_area(self):
        ...
        half_tank_A = tank_A / 2.

        area = rad ** 2 * np.pi + 2 * rad * np.pi * hei

        # test upper and lower tank area
        self.assertAlmostEqual(
            self.tank._tank_area()['upper'],
            half_tank_A,
            places=5)

        self.assertAlmostEqual(
            self.tank._tank_area()['lower'],
            half_tank_A,
            places=5)

        # test whole tank area
        self.assertAlmostEqual(
            self.tank._tank_area(split_tank=False),
            tank_A,
            places=5)
    ...
    def test__thermal_loss(self):
        ...
        Q_loss = self.tank._thermal_transmittance(
            insul_thickness=insul_thick,
            spec_hea_cond=spec_hea_c) * (T_tank - T_amb) * area
    ...
    def test_tap(self):
        ...
        self.assertAlmostEqual(
            self.tank.tap(V_draw_load, T_tank_low, T_feed,
                          T_draw_min=None)['vol'],
            V_draw_load,
            places=5)
        ...
        self.assertTrue(
            self.tank.tap(V_draw_load, T_tank_low, T_feed,
                          T_draw_min=None)['unmet_heat_rate'] > 0.)

        # draw when tank above demand temperature
        T_tank_high = 353.15  # the tank is at 70 degC
        V_high = V_draw_load * (
            T_draw_nom - T_feed) / (T_tank_high - T_feed)

        self.assertAlmostEqual(
            self.tank.tap(V_draw_load, T_tank_high, T_feed,
                          T_draw_min=None)['vol'],
            V_high,
            places=5)
        ...
        Q_at_T_nom = round((T_draw_nom - T_feed) * ro * shc * UnitConv(
            V_draw_load).m3perh_m3pers(unit_in='m3perh'), 2)

        self.assertAlmostEqual(
            self.tank.tap(V_draw_load, T_tank_high, T_feed,
                          T_draw_min=None)['tot_dem'],
            Q_at_T_nom,
            places=2)
    ...
    def test_thermal_tank_dynamics(self):
        ...
        T_tank_upper = 65. + 273.15  # K
        T_tank_lower = 55. + 273.15   # K
        T_amb = 25. + 273.15  # K
        T_main = 18. + 273.15  # K
        Q_in = 500.  # W
        draw_V = .0037  # m3/h
        Q_loss_upper = self.tank._thermal_loss(
            self.tank.therm_transm_coef,
            self.tank.A_upper,
            T_amb, T_tank_upper)
        ...
        steady = self.tank.thermal_tank_dynamics(
            T_amb, T_tank_upper, T_tank_lower,
            Q_in, Q_loss_upper, Q_loss_lower, T_main, Q_tap)
        ...
        self.assertTrue(
            charge_upper[self.r['t_tank_up']] == (
                T_tank_upper + self.tank.dT_approach +
                charge_upper[self.r['t_tank_low']] - T_tank_lower))
        self.assertEqual(charge_upper[self.r['t_tank_up']] - self.tank.dT_approach,
                         charge_upper[self.r['t_tank_low']])

        # Charge both, dQ > 0
        T_tank_upper = 57. + 273.15  # K
        T_tank_lower = 55. + 273.15  # K
        ...
        self.assertEqual(
            charge_both[self.r['t_tank_up']] - charge_both[self.r['t_tank_low']],
        ...
        T_tank_lower = 22. + 273.15  # K

        cool_lower_then_upper = self.tank.thermal_tank_dynamics(
            T_amb, T_tank_upper, T_tank_lower,
            Q_in, Q_loss_upper, Q_loss_lower, T_main, Q_tap)

        self.assertTrue(cool_lower_then_upper[self.r['t_tank_up']] <
                        T_tank_upper)
        self.assertTrue(cool_lower_then_upper[self.r['t_tank_low']] <
                        T_tank_lower)
    ...
    def test__gas_tank_wh(self):
        ...
        tank_input_power = (63560. * tank_V) + 1777.9  # W
        tank_A = 3.3  # m2 (based on dimensions of a 55-gallon tank online)
        tank_U = 1.  # W/m2K (based on default inputs to
        ...
        water_specheat = 4180.  # J/kgK

        # Calculate expected result assuming timestep = 1 h
        term1 = UnitConv(V_draw * water_density * water_specheat *
                         (T_setpoint - T_feed) / tank_re).Wh_J(unit_in='J')
        term2 = 1. - (tank_U * tank_A * (T_setpoint - T_amb) / tank_input_power)
        term3 = tank_U * tank_A * (T_setpoint - T_amb)
        result = (term1 * term2) + term3

        self.assertAlmostEqual(self.tank._gas_tank_wh(
            ...
            water_specheat=water_specheat)[1],
            result,
            places=5)
    ...
    def test_gas_tank_wh(self):
        ...
        hourly_m3 = (hourly_load_df['End-Use Load'].values[0] *
        ...
        df_seasons = pd.DataFrame(['winter', 'winter', 'spring',
        ...
        seasons_and_temps = df_seasons.merge(
            df_T,
            on='season',
            how='left')
        ...
        month_index_hourly['month'] = SourceAndSink(
            input_dfs=inputs).irradiation_and_water_main(
                '16',
                method='isotropic diffuse')['month']

        temps = month_index_hourly.merge(seasons_and_temps,
            on='month',
            how='left')
        ...
        params_gas_tank_wh = pd.DataFrame(
            data=[[self.s['gas_tank'], self.s['tank_re'], .76],
    ...
    def test_thermal_tank(self):
        ...
        size_the_sto = pd.DataFrame(
            data=[[self.s['the_sto'], UnitConv(30).m3_gal()],
                  [self.s['piping'], 20.]],
            columns=[self.s['comp'], self.s['cap']])

        the_sto = Storage(
            params=params_the_sto,
            size=size_the_sto,
            type='sol_tank')
        ...
        self.assertDictEqual(passing_params_in,
            res_no_gain_and_colder_than_set)
    ...
    def test_heat_pump_tank(self):
        ...
        size_the_sto = pd.DataFrame(
            data=[[self.s['the_sto'], UnitConv(80).m3_gal()],
                  [self.s['piping'], 20.]], \
            columns=[self.s['comp'], self.s['cap']])

        the_sto = Storage(
            params=the_sto_params,
            size=size_the_sto,
            type='hp_tank')  # .302833 m3
        ...
        res = the_sto.thermal_tank(
            ...
            pre_V_tap=.00757, \
            pre_Q_in=2350.)  # heat cap of unit A of NRELs heat pump\
...
class DistributionTests(unittest.TestCase):
    ...
    def test__dc_to_ac(self):
        ...
        self.assertTrue(
            Distribution._dc_to_ac(
                in_power,
                conv_eff=total_conv_eff), total_conv_eff * in_power)
    ...
    def test_pipe_losses(self):
        ...
        params_pipes = pd.DataFrame(
            data=[[self.s['piping'], self.s['pipe_spec_hea_con'], .0175],
                  [self.s['piping'], self.s['pipe_ins_thick'], .008],
                  [self.s['piping'], self.s['flow_factor'], .5],
                  [self.s['piping'], self.s['dia_len_exp'], .5],
        ...
        sizes_pipes = pd.DataFrame(
            data=[[self.s['piping'], 20.]],
            columns=[self.s['comp'], self.s['cap']])

        res = Distribution(
            params=params_pipes,
            sizes=sizes_pipes).pipe_losses()
    ...
    def test__pipe_losses(self):
        ...
        loss_heat_rate1, heat_loss1, dT_drop1, flow_on_frac1 = Distribution()._pipe_losses(
                T_in=333.15, T_amb=293.15,
                length=length1, diameter=0.0381,
                insul_thickness=.008, spec_hea_cond=.0175,
                V_tap=.05, max_V_tap=0.1514,
                flow_factor=0.8, circulation=False,
                longest_branch_length_ratio=None)

        loss_heat_rate2, heat_loss2, dT_drop2, flow_on_frac2 = Distribution()._pipe_losses(
                T_in=333.15, T_amb=293.15,
                length=length2, diameter=0.0381,
                insul_thickness=.008, spec_hea_cond=.0175,
                V_tap=.05, max_V_tap=0.1514,
                flow_factor=0.8, circulation=False,
                longest_branch_length_ratio=None)
        ...
        # with branches
        loss_heat_rate1_branches, heat_loss1_branches, \
            dT_drop1_branches, flow_on_frac1_branches = Distribution()._pipe_losses(
                T_in=333.15, T_amb=293.15,
                length=length1, diameter=0.0381,
                insul_thickness=.008, spec_hea_cond=.0175,
                V_tap=.05, max_V_tap=0.1514,
        ...
        # with circulation
        loss_heat_rate1_circ, heat_loss1_circ, \
            dT_drop1_circ, flow_on_frac1_circ = Distribution()._pipe_losses(
                T_in=333.15, T_amb=293.15,
                length=length1, diameter=0.0381,
                insul_thickness=.008, spec_hea_cond=.0175,
                V_tap=.05, max_V_tap=0.1514,
                flow_factor=0.8, circulation=1.,
                longest_branch_length_ratio=None)
        ...
        self.assertEqual(flow_on_frac1_circ, 1.)
    ...
    def test_pump(self):
        ...
        params_pumps = pd.DataFrame(
            data=[[self.s['sol_pump'], self.s['eta_sol_pump'], .8],
        ...
        pump_modeler = Distribution(
            params=params_pumps,
            sizes=sizes_pumps)

        sol_pump_en_use_array, sol_pump_en_use_total = \
            pump_modeler.pump(role='solar')
        dist_pump_en_use_array, dist_pump_en_use_total = \
            pump_modeler.pump(role='distribution')

        self.assertAlmostEqual(
            sol_pump_en_use_total / dist_pump_en_use_total,
            (sizes_pumps.at[0, self.s['cap']] / params_pumps.at[0, self.s['param_value']]) /
            (sizes_pumps.at[1, self.s['cap']] / params_pumps.at[1, self.s['param_value']]),
            places=2)
