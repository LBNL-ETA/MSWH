...
def comp_create(request, component_type):
    Component.objects.create_component(
        name=s[component_type], type=component_type
    )
...
def comp_form(request, component_id):
    ...
    description = (
        request.POST.get("description", "undefined").lstrip().rstrip()
    )
    ...
    if description == "" or description.isspace():
        errors.append(
            "Description field cannot be empty or only whitespace(s)."
        )
...
def config_invoke(request, configuration_id):
    ...
    if errors:
        ...
        try:
            ...
            log.error(
                "No project data found, falling back to default (4 persons)"
            )
        ...
        try:
            ...
            log.error(
                "No climate data found, falling back to default (San Francisco)"
            )
