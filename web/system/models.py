...
from .functions import Map, load_weather, check_params, str2dict, df2dict, dict2df, simulate_system, get_random_project, create_load
...
class Component(models.Model):
    ...
    # Type of the model
    type = models.CharField(max_length=255, choices=Map().get_component_choices(), default='undefined')
...
class ClimateManager(models.Manager):
    ...
    def create_climate(self, name, climate_zone, data_source):
        climate = self.create(name=name, climate_zone=climate_zone, data_source=data_source)
        return climate
...
class Project(models.Model):
    ...
    def get_households(self):
        ...
        for id in range(self.size):
            households.update({id: {'occupancy': self.get_occupancy()[id], 'at_home': self.get_at_home()[id]}})
...
class Configuration(models.Model):
    ...
    type = models.CharField(max_length=255, default='undefined', choices=Map().get_system_choices())
    ...
    # Climate
    climate = models.ForeignKey('Climate', on_delete=models.SET_NULL, blank=True, null=True)
    ...
    def get_params_pd(self, backup=False):
        ...
        component_list = Map().get_system_backup(system=self.type) if backup else Map().get_system_components(system=self.type)

        # Create empty DataFrame for holing the parameters
        params_pd = pd.DataFrame(
            data = [],\
            columns = [s['comp'], s['param'], s['param_value']])
        ...
        return params_pd if len(missing_components) == 0 else missing_components
    ...
    def get_sizes_pd(self, backup=False):
        ...
        if backup:
            sizes_pd = pd.DataFrame(
                data = [],\
                columns = [c['id'], s['comp'], s['cap']])
        else:
            sizes_pd = pd.DataFrame(
                data = [],\
                columns = [s['comp'], s['cap']])
